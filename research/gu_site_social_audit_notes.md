...
- Official Hostinger guide opened: https://www.hostinger.com/support/how-to-deploy-a-nodejs-website-in-hostinger/
- The guide states that Hostinger Node.js Web Apps support Next.js and GitHub integration with automatic builds and deployments on supported Business and Cloud hosting plans. It describes importing a Git repository, selecting or confirming build settings, deploying, and using the Node.js dashboard to review deployment status and restart server-side applications.
- The repository is configured as a Next.js application with `output: 'standalone'`, so the recommended handoff is to deploy the GitHub branch through the Node.js Web App workflow, set `NEXT_PUBLIC_META_PIXEL_ID` in the Hostinger environment, build, restart, and then verify events in Meta Events Manager.
